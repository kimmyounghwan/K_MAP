...
if 'master_data' not in st.session_state:
    with st.spinner("조달청에서 안전하게 최신 공고를 싹 쓸어오는 중입니다... (조금만 기다려주세요!)"):
...
if menu == "📊 실시간 공고 (홈)":
    ...
    if not df.empty:
        # 🚨 [명환이 지시사항] 최신 날짜 무조건 1등으로 올리기 (에러값은 맨 밑으로!)
        ...
        def get_safe_link(row):
            if 'bidNtceDtlUrl' in row and pd.notna(row['bidNtceDtlUrl']) and str(row['bidNtceDtlUrl']).strip() != "":
                ...
                return f"https://www.g2b.go.kr/ep/invitation/publish/bidInfoDtl.do?bidno={row['bidNtceNo']}&bidseq=00"
        ...
        with col1:
            st.metric(label="누적 공고(최근 데이터)", value=f"{len(df):,}건")
        with col2:
