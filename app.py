...
def update_stats():
    try:
        ...
        if 'visited' not in st.session_state:
            ...
            t_v = db.child("stats").child("total_visits").get().val() or 802
...
def get_hybrid_1st_bids():
    ...
    # [핵심 수술] 인덱스 에러 방지를 위해 order_by_key() 사용 후 파이썬에서 정렬
        ...
        try:
            ...
            if len(corp) > 1 and corp[0].strip():
                new_rows[bid_no] = {
                    ...
                    '투찰률': f"{corp[4].strip()}%" if len(corp) >= 5 else '-', '전체업체': full_corp_info
                }
        except:
            continue
...
if menu == "🏆 1순위 현황판":
    ...
    if posts:
        for post_id, p in reversed(list(posts.items())):
            with st.expander(f"📢 {p.get('title')} (작성자: {p.get('author')})"):
                st.write(p.get('content'))
                if st.session_state['user_name'] in [p.get('author'), "명환"]:
                    if st.button("🗑️ 삭제", key=f"del_{post_id}"): db.child("posts").child(post_id).remove(); st.rerun()
    ...
    if not st.session_state['logged_in']:
        ...
        chats = db.child("k_chat").limit_to_last(30).get().val()
        if chats:
    ...
    with t2:
        ...
        if st.button("가입하기"):
            try:
                ...
                st.error("가입 실패! 이미 있는 계정이거나 형식이 잘못되었습니다.")